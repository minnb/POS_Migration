...
| Trang Ops `/ops/health` | `HealthCheckService.CheckWorkerHeartbeatAsync` — đọc key `Worker:Heartbeat:{HealthCheck:WorkerName}` (mặc định **chỉ `"PosSalesConsumer"`**, config `src/POS.Web/appsettings.json:91`), báo "Đang chạy" nếu nhịp cuối ≤45s, "Suy giảm" nếu `Status=Degraded`, "Mất tín hiệu" nếu quá 45s không có nhịp | Chỉ 1 worker/lần theo config — xem giới hạn ở mục 5 |
