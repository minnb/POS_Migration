/* ============================================================================
   8.2 Phát hành THÊM coupon (nhiều lô) — SP thêm 1 lô mã Auto MỚI vào coupon
   ĐÃ TỒN TẠI, KHÔNG đổi header, KHÔNG guard tồn tại (khác usp_SetupCoupon_SaveIssue
   ...
   Khớp usp_SetupVoucher_IssueMore.sql — tái dùng dbo.CouponCodeTVP đã tạo ở SetupCoupon_Save.sql.
...
IF OBJECT_ID(N'dbo.usp_SetupCoupon_IssueMore', N'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_SetupCoupon_IssueMore;
GO

CREATE PROCEDURE dbo.usp_SetupCoupon_IssueMore
(
...
BEGIN
    ...
    BEGIN
        THROW 50002, N'Không tìm thấy coupon (ItemNo không tồn tại)', 1;
    END
    BEGIN TRY
        BEGIN TRANSACTION;
        ...
        SELECT @ItemNo, t.Code, 1, @now, @cntCode + 1, @ItemNo, 'COUPON',
               ...
               'VND', 'WCM', 'SOLD', 0, CAST(h.DiscountValue AS decimal(18,2)), h.CpnVchType
        FROM   @Codes t
        CROSS JOIN (SELECT ArticleType, StartingDate, EndingDate, DiscountValue, CpnVchType
