...
Auth của POS.Api (không ảnh hưởng nhau). 3 role cố định (không có UI tạo role mới — role là
...
3 role map vào **3 policy phân cấp** (role cao hơn tự động có quyền của role thấp hơn):

| Policy (`WebPolicies`) | Role được phép | Dùng cho |
|---|---|---|
| `StoreAndAbove` | `StoreOperator`, `ITOps`, `SystemAdmin` | Trang cấp cửa hàng (báo cáo, giao dịch, vận hành ca) |
| `OpsAndAbove` | `ITOps`, `SystemAdmin` | Trang cấu hình danh mục/khuyến mãi + giám sát hệ thống |
...
> phân quyền chỉ theo **3 role cố định → 3 policy cố định → route-level `[Authorize]`**. Muốn
...
| `Role` | `NVARCHAR(50)` | `'StoreOperator'` \| `'ITOps'` \| `'SystemAdmin'` — **string tự do, không FK/CHECK constraint** |
...
- **Role là chuỗi tự do trong DB** — validate hợp lệ chỉ ở tầng UI (`UsersPage.razor` dùng
  `MudSelect` với 3 `MudSelectItem` cố định theo `WebRoles.*`, không cho nhập tự do). Không có
...
### 5.2 `OpsAndAbove` — ITOps + SystemAdmin (Danh mục / Khuyến mãi / Vận hành)
...
### 5.3 `AdminOnly` — chỉ SystemAdmin (Quản trị)
...
2. Nút "Thêm mới" → điền `Username`/`FullName`/`Password` → chọn **Role** (dropdown 3 giá trị cố
   định: Cửa hàng / IT Ops / System Admin) → nếu Role = Cửa hàng, có thể giới hạn **Store codes**.
...
- **Role phải khớp chính xác** 1 trong 3 chuỗi: `StoreOperator` / `ITOps` / `SystemAdmin` (phân
  biệt hoa/thường, không có khoảng trắng) — sai chính tả sẽ khiến `RequireRole` không match, user
  bị chặn ở mọi trang `[Authorize]` mà không có thông báo lỗi rõ ràng (chỉ redirect
  `/access-denied`).

### 6.3 Thêm quyền cho 1 page mới (dành cho dev)

1. Xác định page thuộc nhóm nào theo bảng §5 → chọn đúng policy (`StoreAndAbove` /
   `OpsAndAbove` / `AdminOnly`).
...
5. Không cần đăng ký gì thêm ở tầng policy/role — 3 policy đã cố định, page mới chỉ chọn 1 trong 3.

### 6.4 Muốn thêm role mới / permission mịn hơn (chưa có sẵn — cần thiết kế)

Hiện KHÔNG có cơ chế thêm role thứ 4 hoặc permission per-action mà không sửa code:
- Thêm hằng số role mới trong `WebRoles`.
- Thêm/sửa `AddPolicy` + `RequireRole` trong `Program.cs`.
- Cập nhật `MudSelectItem` role trong `UsersPage.razor`.
- Rà lại toàn bộ 59 page — quyết định policy nào áp dụng cho role mới.
- Cân nhắc điền nội dung thật cho `/admin/roles` (`RolesPage.razor`) nếu muốn UI quản lý role
  động thay vì hằng số cứng trong code.
