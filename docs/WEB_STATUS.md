...
> `IDirectoryProbe`, cấu hình `Logging:LogDirectory` tường minh (4 appsettings), và hạ tầng
> systemd cho POS.Api/POS.Web (UAT/PROD thật chạy native, không Docker — chỉ POS.Worker Docker).
> Xem chi tiết đầy đủ ở entry L1 trong bảng bên dưới + `docs/CHANGELOG.md`. Build 0 error, 34/34
> `POS.UnitTests` (mới, gồm 10 test `LogFileServiceTests` chạy thật trên filesystem) + 45/45
> `POS.ContractTests` xanh; đã tự chạy `dotnet run` thật, xác nhận `GET /admin/logs` → 302 redirect
> `/login` đúng hành vi, `/health` → 200, không exception — vẫn CHƯA test tay qua trình duyệt thật
> (cần phiên đăng nhập SystemAdmin qua UI).
...
| S5 | Mã hóa credentials appsettings (C4) — POS.Web + POS.Api + POS.Worker | SecretProtector.cs + ConfigurationSecretExtensions.cs + Program.cs (3 project) + EncryptSecretPage.razor | ✅ (cơ chế) ✅ (đã rollout Production cho Api/Web) ⚠️ (Worker: có hook, file Production **còn plaintext**) | AES-256-GCM token `enc:`, khóa chung `POS_SECRET_KEY`; hook giải mã rút thành extension chung `DecryptEncryptedSecrets()` (2026-07-10), wired ở CẢ POS.Api, POS.Web **và POS.Worker**; trang `/admin/encrypt-secret` (POS.Web) sinh token cho cả 3. `appsettings.Production.json` của POS.Api + POS.Web đã mã hóa (9 connection string + RabbitMQ); **POS.Worker chưa mã hóa** — hook no-op cho tới khi ops thay `Password=enc:...` (khi đó môi trường Worker phải có `POS_SECRET_KEY`). Key đã ghi vào `.env` local — **UAT/PROD server thật vẫn cần tự set `POS_SECRET_KEY`**. Tra cứu nhanh: `docs/architecture/appsetting.md`; chi tiết: `docs/ROLLOUT.md` |
...
| L1 | LogFilePage – /admin/logs + AdminOnly + InteractiveServer | Pages/Admin/LogFilePage.razor | ✅ | Quản lý Log Server — **đổi 2026-07-13 (lần 3): bỏ hẳn `MudTreeView` (lần 2, 2026-07-11), quay lại breadcrumb (`MudLink` thủ công) + `MudList` drill-down** (xem `.claude/skills/web/component-patterns.md` mục 4b) — vì hierarchy log chỉ nông 2-3 cấp cố định (`api`/`web`/`worker`), TreeView tự tạo lớp bug "expand nuốt click select" không cần thiết cho use-case này. Click 1 `MudListItem` = `LoadAsync(folder.RelativePath)` — load lại CẢ folder-list lẫn file-table cho path đó trong 1 hành động, không còn khái niệm expand/select tách biệt. Backend **không đổi API** — vẫn chỉ `GetDirectoryListingAsync(relativePath)` (trả folders+files của đúng 1 cấp, tái dùng thẳng cho breadcrumb) + `DownloadLogFileAsync`; không có `GetSubfoldersAsync` nào (method đó không tồn tại trong code hiện tại — dòng trước đây có nhắc tới nó là sai). **Cùng đợt — production-hardening**: (1) `Logging:LogDirectory` tường minh trong cả 4 `appsettings*.json` (thay suy luận ngầm thư mục cha của `FileLogDirectory`); (2) `LogFileDownload.Content` đổi `byte[]`→`Stream`, `DownloadLogFileAsync` trả `FileStream` thật (không buffer RAM), `JsDownloadExtensions` thêm overload `Stream` cho `DotNetStreamReference`; (3) `IDirectoryProbe`/`PhysicalDirectoryProbe` (mới, `Services/IDirectoryProbe.cs`) — seam I/O cho phép test giả lập `UnauthorizedAccessException` + `IsSymbolicLink` (chống Symlink Attack — lọc symlink khỏi listing, chặn download); (4) hạ tầng: `deploy/linux/systemd/{pos-api,pos-web}.service` + `deploy/linux/setup-pos-log-dirs.sh` (group `posops` dùng chung, setgid 2750) cho topology thật UAT/PROD (POS.Api/POS.Web native systemd, chỉ POS.Worker Docker) — xem `docs/guide-deploy.md` + `docs/ROLLOUT.md` §O12; (5) `Program.cs` thêm `PersistKeysToFileSystem` tường minh cho Data Protection Keys (gap phát hiện khi audit — trước đó dựa mặc định `$HOME`, rủi ro mất session sau mỗi lần restart systemd). Verify: `dotnet build` 0 error + `tests/POS.UnitTests` 34/34 xanh (10 test mới `LogFileServiceTests` — gồm 1 test chạy THẬT trên `D:\ROOT\Logs`, không mock) + `tests/POS.ContractTests` 45/45 xanh + tự `dotnet run` thật xác nhận `GET /admin/logs` → 302 `/login` đúng hành vi, `/health` → 200, không exception — **vẫn chưa test tay qua trình duyệt thật** (cần phiên đăng nhập SystemAdmin qua UI, môi trường hiện tại không có browser). |
