# POS.Web — Kiến trúc & Chuẩn nền tảng (Reference đầy đủ)

> Nguồn canonical đầy đủ cho auth flow, roles/policy, services inject được, template page,
> breaking changes MudBlazor v9, responsive/density/typography, tổ chức thư mục — được
> `.claude/rules/02-blazor-mudblazor-ui.md` (luật thép, đọc trước) và `.claude/skills/web/SKILL.md`
> (index) trỏ tới. Số mục (§N) giữ nguyên qua các lần sửa — các skill khác trích dẫn theo số mục
> này, không đổi số khi chỉ sửa nội dung bên trong.
...
- Chi tiết font-size/font-weight/lịch sử: `.claude/skills/web/theme-reference.md` mục 6.
...
## 12. Slash Commands / Skills (POS.Web)
...
| `/web-ops gen-hash` | Tạo BCrypt hash cho user migration SQL |
| `/add-dto-common` | Thêm DTO mới vào POS.Common |
...
(xem khuôn mẫu chuẩn đầy đủ ở `.claude/skills/web/theme-reference.md` mục 11 "KPI card").
...
## 17. Performance, DataTable & Component standards
...
| Số liệu tổng quan (KPI card) | `MudPaper` + `.pos-kpi-value`/`.pos-kpi-label` (khuôn mẫu mục 11 `theme-reference.md`) |
...
| Badge trạng thái tĩnh | `<span class="pos-status-chip pos-status-{success,error,warning,info}">` (§4a `theme-reference.md`) |
...
> `button.btn-primary`) → dùng bảng ở `theme-reference.md` mục 0.
